...
class PaintRenderer extends BaseTextRenderer {
    public PaintRenderer(int fontSize, ColorScheme scheme) {
        this(fontSize, scheme, null);
    }

    public PaintRenderer(int fontSize, ColorScheme scheme, String fontPath) {
        super(scheme);
    ...
    public void drawTextRun(Canvas canvas, float x, float y, int lineOffset,
        ...
        float cursorX = 0;
        float actualTextWidthBeforeCursor = 0;
        if (cursorVisible) {
            int countBeforeCursorChars = cursorIndex - index;
            if (countBeforeCursorChars > 0 && countBeforeCursorChars <= count) {
                actualTextWidthBeforeCursor = mTextPaint.measureText(text, index, countBeforeCursorChars);
            }
            cursorX = left + actualTextWidthBeforeCursor;
            
            // We also need to measure the cursor width itself
            float actualCursorCharWidth = mCharWidth;
            if (cursorIncr > 0 && cursorIndex + cursorIncr <= index + count) {
                actualCursorCharWidth = mTextPaint.measureText(text, cursorIndex, cursorIncr);
            }
            drawCursorImp(canvas, (int) cursorX, y, actualCursorCharWidth, mCharHeight, cursorMode);
        }
        ...
        if (!invisible) {
            ...
            android.util.Log.i("PaintRenderer", "Drawing text: " + new String(text, index, count) + " at " + left + "," + (y - mCharDescent) + " with color " + Integer.toHexString(textPaintColor));
            ...
            if (cursorVisible) {
                ...
                if (countAfterCursor > 0) {
                    mTextPaint.setColor(textPaintColor);
                    float actualCursorWidth = mTextPaint.measureText(text, cursorIndex, cursorIncr);
                    canvas.drawText(text, cursorIndex + cursorIncr, countAfterCursor,
                            cursorX + actualCursorWidth,
                            textOriginY, mTextPaint);
                }
            } else {
