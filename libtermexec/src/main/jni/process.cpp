...
public:
    ...
    void set(const jchar* o, size_t numChars) {
        ...
        int charsAllocated = numChars;
        char* buf = (char*) malloc(numChars + 1);
        if (!buf) {
            return;
        }
        char* p = buf;
        for (size_t i = 0; i < numChars; i++) {
            jchar c = o[i];
            if (c > 127) {
                // ... handle unicode? this is a hack anyway
            }
            *p++ = (char) c;
        }
        *p = '\0';
...
static int create_subprocess(JNIEnv *env, const char *cmd, char *const argv[], char *const envp[], int masterFd)
{
    ...
    // grantpt is necessary on modern Linux/Android to set up the pty
    if(grantpt(masterFd)){
        throwIOException(env, errno, "trouble with grantpt");
        return -1;
    }
    ...
    if(pid == 0){
        ...
        // closeNonstandardFileDescriptors(); // May break exec on modern Android
